...
@register_meta_arch("LocPointTransformerMM1")
class PtTransformer(nn.Module):
    ...
    def __init__(
            ...
            fused_embd,  # if to fuse the embedding network with the stem network
        ...
        assert backbone_type in ['convTransformer', 'conv', 'convTransformerMM1', 'convTransformerMM2']
        ...
        elif backbone_type == 'convTransformerMM1':
            self.backbone = make_backbone(
                'convTransformerMM1',
                **{
                    'n_in': input_dim,
        ...
        elif backbone_type == 'convTransformerMM2':
            self.backbone = make_backbone(
                'convTransformerMM2',
                **{
                    'n_in': input_dim,
    ...
    def forward(self, video_list, save_output_path=None, save_fm_path=None):
    ...
    def inference(
        ...
        for idx, (vidx, fps, vlen, stride, nframes) in enumerate(
            ...
            if save_fm_path is not None:
                with open(os.path.join(save_fm_path, vidx + '.pkl'), 'wb') as f:
                    pickle.dump(results_per_vid, f)
        # step 3: postprocssing
        results = self.postprocessing(results, save_output_path=save_output_path)
    ...
    @torch.no_grad()
    def postprocessing(self, results, save_output_path=None):
        ...
        for results_per_vid in results:
            ...
            if save_output_path is not None:
                np.savez(os.path.join(save_output_path, vidx + '.npz'), segs=segs, scores=scores, labels=labels,
                         fps=fps, vlen=vlen, stride=stride, nframes=nframes)
