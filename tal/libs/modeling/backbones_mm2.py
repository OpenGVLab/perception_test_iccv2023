...
class AttentionPooling(nn.Module):
    def __init__(self, dim=1024, num_heads=16):
        ...
        self.num_queries = 1
        self.query = nn.Embedding(self.num_queries, dim)
    ...
    def forward(self, x):
        # x: batch size, num_features, feature channel, sequence length,
        x = x.permute(0, 3, 1, 2)
        B, T, nf, C = x.size()
        x = x.reshape(B * T, nf, C)
        q = self.query.weight.unsqueeze(0).repeat(B * T, 1, 1)
        # x = self.norm(x)
        k = self.k(x)
        v = self.v(x)
        q = q.reshape(B * T, self.num_queries, self.num_heads, C // self.num_heads).transpose(1, 2)
        k = k.reshape(B * T, nf, self.num_heads, C // self.num_heads).transpose(1, 2)
        v = v.reshape(B * T, nf, self.num_heads, C // self.num_heads).transpose(1, 2)

        out = F.scaled_dot_product_attention(q, k, v)
        out = out.transpose(1, 2).reshape(B * T, self.num_queries, C).reshape(B, T, C)
        out = self.o(out).transpose(1, 2)

        return out


@register_backbone("convTransformerMM2")
class ConvTransformerBackbone(nn.Module):
    ...
    def __init__(
            ...
            fused_embd=None,  # fused embedding dimension (after fusion)
        ...
        if isinstance(n_in, (list, tuple)):
            ...
            self.proj = None
        n_in = fused_embd
        self.attn_pool = AttentionPooling(dim=fused_embd, num_heads=n_head)
