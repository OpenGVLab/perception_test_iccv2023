...
from . import transforms as video_trans
import decord


@register_dataset("perception_e2e")
class ActivityNetDataset(Dataset):
    def __init__(
            self,
            is_training,  # if3 in training mode
        ...
        if not is_training:
            self.transforms = trans.Compose([
                trans.Resize(transforms_cfg.get("input_size")),
                video_trans.CenterCropVideo(transforms_cfg.get("crop_size"), ),
                video_trans.NormalizeVideo(mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375])
            ])
        else:
            self.transforms = trans.Compose([
                trans.Resize(transforms_cfg.get("input_size")),
