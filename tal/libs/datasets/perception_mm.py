...
@register_dataset("perception_tal_mm")
class ActivityNetDataset(Dataset):
    def __init__(
            self,
            is_training,  # if in training mode
            split,  # split, a tuple/list allowing concat of subsets
            feat_folder_list: list,  # folder for features
            json_file,  # json file for annotations
            feat_stride,  # temporal stride of the feats
            num_frames,  # number of frames for each feat
            feat_stride_list: list,  # temporal stride of the feats
            num_frames_list: list,  # number of frames for each feat
            ...
            file_ext_list: list,  # feature file extension if any
        ...
        # file path
        for feat_folder in feat_folder_list:
        ...
        self.feat_folder_list = feat_folder_list
        ...
        if file_prefix is not None:
            ...
            self.file_prefix = ''
        self.file_ext_list = file_ext_list
        ...
        # features meta info
        self.feat_stride_list = feat_stride_list
        self.num_frames_list = num_frames_list
    ...
    def __getitem__(self, idx):
        ...
        feats_list = []
        for i, feat_folder in enumerate(self.feat_folder_list):
            filename = os.path.join(feat_folder + '/'
                                    + video_item['id'] + self.file_ext_list[i])
            if os.path.exists(filename):
                if self.file_ext_list[i] == ".npy":
                    feats = np.load(filename).astype(np.float32)
                elif self.file_ext_list[i] == ".pt":
                    feats = torch.load(filename).numpy().astype(np.float32)
                else:
                    raise NotImplementedError
            else:
                # print("feat not found:", filename, flush=True)
                feats = np.zeros((1, self.input_dim[i]), dtype=np.float32)
            # print(feats.shape)
            feats_list.append(feats)
        # print("---------------")
        max_feat_len = max([feats.shape[0] for feats in feats_list])
        max_feat_idx = [feats.shape[0] for feats in feats_list].index(max_feat_len)
        self.feat_stride = self.feat_stride_list[max_feat_idx]
        self.num_frames = self.num_frames_list[max_feat_idx]

        for i, feats in enumerate(feats_list):
            feats_list[i] = F.interpolate(torch.from_numpy(feats).unsqueeze(0).transpose(1, 2), size=max_feat_len,
                                          mode='linear',
                                          align_corners=False).squeeze(0).transpose(0, 1).numpy()
        ...
        if self.input_modality == 'multi':
            ...
            try:
                ...
                xnew = np.linspace(x.min(), x.max(), feats.shape[0] * scale_keep)
                # apply the interpolation to each column
                f = interp1d(x, mm_feats, axis=0)
                mm_feats = f(xnew)
            except Exception:
                print("mm_feat not error:", mm_filename, flush=True)
                mm_feats = np.zeros((feats.shape[0] * scale_keep, 768))

            mm_feats = mm_feats.reshape((feats.shape[0], scale_keep * 768))
