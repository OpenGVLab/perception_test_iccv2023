...
def make_optimizer(model, optimizer_config):
    ...
    blacklist_weight_modules = (LayerNorm, torch.nn.GroupNorm, torch.nn.Embedding, torch.nn.LayerNorm)
...
def valid_one_epoch(
    ...
    for iter_idx, video_list in enumerate(val_loader, 0):
        ...
        with torch.no_grad():
            save_output_path = output_file.replace(".json", "_inter")
            save_fm_path = output_file.replace(".json", "_inter_fm")
            mmengine.mkdir_or_exist(save_output_path)
            mmengine.mkdir_or_exist(save_fm_path)
            output = model(video_list, save_fm_path=save_fm_path)
    ...
    if evaluation:
        ...
        # save the results that can directly feed into the official evaluation code
        # inter_output_file = output_file.replace('.json', f'_mAP={mAP}.pkl')
        # with open(inter_output_file, 'wb') as my_file:
